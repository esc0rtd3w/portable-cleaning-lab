...
set appName=ClamWin Antivirus
set appBin=freshclam.exe
set appVersion=0.98.5
:: Next Line Only Needed If All Users Folder Is Used
set pathUpdatesLocalNameAU=.clamwin
:: Next Line Only Needed If Current User Folder Is Used
set pathUpdatesLocalNameCU=.clamwin
...
:: Default Scanner EXE
set binScanner="%pathCoreLocalRun%\core\bin\%appBin%"
set binArgs=--verbose
set binUpdate="%pathCoreLocalRun%\core\bin\%appBin%"
set binUpdateSilent="%pathCoreLocalRun%\core\bin\%appBin%"
...
:: Copy files from ALLUSERS folder for running commandline options ONLY
%mkdir% "%pathCoreLocalRun%\core\bin\db"
%copy% "%pathUpdatesLocalAU%\db\*" "%pathCoreLocalRun%\core\bin\db"

:: Write FreshClam.conf file with extracted path or update will fail
echo DatabaseDirectory "%pathCoreLocalRun%\core\bin\db\">"%pathCoreLocalRun%\core\bin\FreshClam.conf"
echo DatabaseMirror database.clamav.net>>"%pathCoreLocalRun%\core\bin\FreshClam.conf"
:: Next line fucks shit up for some reason?!?!?!?!? Its not needed anyways for update to work
::echo MaxAttempts 3>>"%pathCoreLocalRun%\core\bin\FreshClam.conf"
...
::----------------------------------------------------------------------------
:: Definition Repack Stuff No Check
::%pack7z% "%temp%\definitions.zip" "%pathUpdatesLocalTempCU%" -o"%temp%\"
::%copy% "%temp%\definitions.zip" "%pathCoreLocalRun%\core\definitions.zip"
::----------------------------------------------------------------------------
...
%rm% "%pathCoreLocalRun%\core\bin\FreshClam.conf"

:: Copy files from db back to ALL USERS folder for repack
%copy% "%pathCoreLocalRun%\core\bin\db\*" "%pathUpdatesLocalAU%\db"

:: Remove db files from core so they dont get packed
%rm% "%pathCoreLocalRun%\core\bin\db\bytecode.cld"
%rm% "%pathCoreLocalRun%\core\bin\db\bytecode.cvd"
%rm% "%pathCoreLocalRun%\core\bin\db\daily.cvd"
%rm% "%pathCoreLocalRun%\core\bin\db\main.cvd"
%rm% "%pathCoreLocalRun%\core\bin\db\mirrors.dat"
