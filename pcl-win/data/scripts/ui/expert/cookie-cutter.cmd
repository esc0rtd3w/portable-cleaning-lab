...
color 0d
for /f "tokens=*" %%a in ('dir /b "%cookies%"') do (
    set /a cookieCutter=!cookieCutter! + 1
    set var!cookieCutter!=%%a
	echo "%cookies%\%%a"
	%rm% "%cookies%\%%a"
)
endlocal
...
:: Cookies Low Current User
color 0d
for /f "tokens=*" %%a in ('dir /b "%cookiesLow%"') do (
    set /a cookieCutterLow=!cookieCutterLow! + 1
    set var!cookieCutterLow!=%%a
	echo "%cookiesLow%\%%a"
	%rm% "%cookiesLow%\%%a"
)
endlocal
